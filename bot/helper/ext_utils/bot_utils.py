        ...
        for download in list(download_dict.values()):
            ...
            if INDEX > COUNT:               
                ...
                if download.status() not in [
                    ...
                    msg += f" <code>{get_progress_bar_string(download)} {download.progress()}</code>"
                    if download.status() == MirrorStatus.STATUS_DOWNLOADING:
                        msg += f"\n\n<b>Downloaded:</b> {get_readable_file_size(download.processed_bytes())}<b>\nSize</b>: {download.size()}"
                    elif download.status() == MirrorStatus.STATUS_CLONING:
                        msg += f"\n\n<b>Cloning:</b> {get_readable_file_size(download.processed_bytes())} \n<b>Size</b>:: {download.size()}"
                    else:
                    ...
                    try:
                        ...
                        pass
                    try:
                        msg += f"\n<b>Engine: Aria2</b> <b>📶:</b> {download.aria_download().connections}"
                    except:
                        pass
                    try:

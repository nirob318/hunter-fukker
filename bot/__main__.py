...
def stats(update, context):
    ...
    current = now.strftime('Date: %d/%m/%Y\nTime: %I:%M:%S %p')
...
'''
    ...
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        ...
        sendMessage(f"Not a Authorized user.\nUse Me In <a href='https://t.me/+y4ZNKvn-mYo2N2Nk'>Superior Mirror</a>", context.bot, update)
...
def main():
    ...
    if GROUP_ID is not None and isinstance(GROUP_ID, str):        
        try:
            dispatcher.bot.sendMessage(f"{GROUP_ID}", f"Bot Restarted\n{jam}\n\nTimezone: {TIMEZONE}\n")
        except Unauthorized:
            LOGGER.warning(
...
# Heroku restarted
    ...
    if GROUP_ID2 is not None and isinstance(GROUP_ID2, str):        
        try:
            dispatcher.bot.sendMessage(f"{GROUP_ID2}", f"Bot Restarted\n{jam}\n\nTimezone: {TIMEZONE}\n")
        except Unauthorized:
            LOGGER.warning(
                "Bot isnt able to send message to support_chat, go and check!"
            )
